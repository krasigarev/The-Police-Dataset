...
- df.isnull( ).sum( ) - It detects the missing values from each column of the dataframe. df.drop(‘Col_name’ ) - To drop a column from dataframe.
...
- df['Column_name'].mean() - To show Mean value of a column. df.groupby('Column_1').Column_2.describe() - To create groups based on Column1 and show statistics summary based on Column2.
...
- Groupby and Describe - Compare the age distributions for each violation.
